//
//  TuesdayForRegisterViewController.swift
//  LessonProgram
//
...
final class TuesdayForRegisterViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if isThereAProgramForTheDay(day: "salı") {
    ...
    @IBAction func saveLessons(_ sender: Any) {
        ...
        if let lesson1 = lesson1Text, let lesson = lesson1.text, lesson != ""{
            ...
            self.createNewLesson(name: lesson, time: 1, day: "salı")
            
        }else{
            if (isThisRowFull(time: 1, day: "salı")){
                self.createNewLesson(name: "", time: 1, day: "salı")
            }
        }
        if let lesson2 = lesson2Text, let lesson = lesson2.text, lesson != ""{
            
            self.createNewLesson(name: lesson, time: 2, day: "salı")
            
        }else{
            if (isThisRowFull(time: 2, day: "salı")){
                self.createNewLesson(name: "", time: 2, day: "salı")
            }
        }
        if let lesson3 = lesson3Text, let lesson = lesson3.text, lesson != ""{
            
            self.createNewLesson(name: lesson, time: 3, day: "salı")
            
        }else{
            if (isThisRowFull(time: 3, day: "salı")){
                self.createNewLesson(name: "", time: 3, day: "salı")
            }
        }
        if let lesson4 = lesson4Text, let lesson = lesson4.text, lesson != ""{
            
            self.createNewLesson(name: lesson, time: 4, day: "salı")
            
        }else{
            if (isThisRowFull(time: 4, day: "salı")){
                self.createNewLesson(name: "", time: 4, day: "salı")
            }
        }
        if let lesson5 = lesson5Text, let lesson = lesson5.text, lesson != ""{
            
            self.createNewLesson(name: lesson, time: 5, day: "salı")
            
        }else{
            if (isThisRowFull(time: 5, day: "salı")){
                self.createNewLesson(name: "", time: 5, day: "salı")
            }
        }
        if let lesson6 = lesson6Text, let lesson = lesson6.text, lesson != ""{
            
            self.createNewLesson(name: lesson, time: 6, day: "salı")
            
        }else{
            if (isThisRowFull(time: 6, day: "salı")){
                self.createNewLesson(name: "", time: 6, day: "salı")
            }
        }
        if let lesson7 = lesson7Text, let lesson = lesson7.text, lesson != ""{
            
            self.createNewLesson(name: lesson, time: 7, day: "salı")
            
        }else{
            if (isThisRowFull(time: 7, day: "salı")){
                self.createNewLesson(name: "", time: 7, day: "salı")
            }
        }
        if let lesson8 = lesson8Text, let lesson = lesson8.text, lesson != ""{
            
            self.createNewLesson(name: lesson, time: 8, day: "salı")
            
        }else{
            if (isThisRowFull(time: 8, day: "salı")){
                self.createNewLesson(name: "", time: 8, day: "salı")
            }
        }
        resultLabel.text = "Kaydedildi"
        resultLabel.backgroundColor = .green
        ...
        userDefaults.set(true, forKey: "salı")
        ...
        if (isThereAProgramForTheDay(day: "pazartesi") && isThereAProgramForTheDay(day: "salı") && isThereAProgramForTheDay(day: "çarşamba") && isThereAProgramForTheDay(day: "perşembe") && isThereAProgramForTheDay(day: "cuma")){
            performSegue(withIdentifier: "tuesdayToMonday", sender: nil)
        }
    }
    ...
    func readFromDatabaseForRows(){
        ...
        let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: "Lessons")
        fetchRequest.predicate = NSPredicate(format: "lesson_day == %@", "salı")
